...
reasoning_output_tokens       = reasoning tokens (Codex/every-code/OmO/Cline fold them into output_tokens for cost)
total_tokens                  = input + output + cache_creation + cache_read (+ reasoning_output when output excludes reasoning); Codex/OmO/Cline keep reasoning as an informational subset because it is already included in output_tokens
```
